...
class AuthService:
    ...
    def build_token(self, account_id: str) -> TokenSchema:
        ...
        expiration_date = datetime.now(UTC) + timedelta(
            minutes=base_config.midaas_user_token_time_minutes
        )
        ...
        jwt_token = jwt.encode(
            jwt_token_body, self._jwt_secret, algorithm=self._jwt_algorithm
        )
        return TokenSchema(token=jwt_token)

    def get_account(self, jwt_token: str) -> BankAccount:
        decoded_data = jwt.decode(
            jwt_token, self._jwt_secret, algorithms=self._jwt_algorithm
        )
